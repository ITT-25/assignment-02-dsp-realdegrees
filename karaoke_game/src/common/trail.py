...
class Trail:
    def __init__(self):
        ...
        self.history: Deque[Optional[tuple[float, float]]] = deque(
            maxlen=Config.PLAY_LINE
        )

    def update(
        self, delta_time: float, cursor_active: bool, current_y: Optional[float]
    ):  # Changed: history to current_y, type to Optional[float]
        # Update history
        ...
        for i in range(len(self.history)):
            if self.history[i] is not None:
                self.history[i] = (
                    self.history[i][0] - Config.SCROLL_SPEED * delta_time,
                    self.history[i][1],
                )

        # Split the history into segments between None values
        self.segments = [
            MultiLine(*segment, thickness=3, color=(255, 255, 255), batch=Config.BATCH)
            for segment in self._get_history_segments()
        ]
